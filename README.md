<!--
  GitHub Profile — Fidel Alejandro Fernandez Arias (@ale94lko)
-->

<p align="center">
  <img src="./assets/header.svg" alt="Fidel Alejandro Fernandez — Fullstack Developer" width="100%" />
</p>

<p align="center">
  <a href="mailto:ale94lko@gmail.com"><img src="https://img.shields.io/badge/Email-ale94lko%40gmail.com-0c1828?style=for-the-badge&logo=gmail&logoColor=e0b07a&labelColor=0a1628" alt="Email" /></a>
  <a href="https://github.com/ale94lko?tab=repositories"><img src="https://img.shields.io/badge/GitHub-ale94lko-c4894a?style=for-the-badge&logo=github&logoColor=ffffff&labelColor=0a1628" alt="GitHub" /></a>
  <img src="https://komarev.com/ghpvc/?username=ale94lko&style=for-the-badge&color=3d9b8f&label=PROFILE+VIEWS" alt="Profile views" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3500&pause=900&color=E0B07A&center=true&vCenter=true&width=720&lines=Fullstack+Developer;Developer+Tools+%26+GitHub+Actions;SQL+%C2%B7+PHP+%C2%B7+TypeScript;Let's+be+realistic+and+do+the+impossible" alt="Typing headline" />
</p>

---

## About

Computer Science Engineer (graduated **with Honours**, GPA **4.79**) based in **Cuba**.  
I build fullstack products and open-source **developer tools** — GitHub Actions, SQL quality, queue reliability, and local-first LLM workflows.

> *Let's be realistic and do the impossible.*

| | |
|---|---|
| **Role** | Fullstack Developer · Open Source Maintainer |
| **Focus** | Backend systems, DX tooling, CI automation, data integrity |
| **Languages** | Spanish (native) · English (intermediate) |
| **Status** | Available for opportunities |

---

## Experience

<table>
<tr>
<td width="28%"><strong>2021 — Present</strong><br/><em>Remote / UAE</em></td>
<td>

### Freelance — Fullstack Developer
Open-source contributor and maintainer.
- Contributions to **MDN Web Docs**, **Vanilla Select Box**, and **The Algorithms — PHP**
- Author of developer tools used by the community (`php-cs-fixer-action`, `sql-optima`, `llm-workbench` and more)

</td>
</tr>
<tr>
<td><strong>2021 — Present</strong><br/><em>First Due Size Up</em></td>
<td>

### Fullstack Developer — Incident Reporting (FDSU)
- Bug fixing across application and database layers
- New features, reports, and full modules for Canadian clients
- Pull request reviews for the engineering team
- **Stack:** PHP · Yii · Vue · PostgreSQL · Storybook

</td>
</tr>
<tr>
<td><strong>2020 — 2021</strong><br/><em>OPTIMA COMPASS</em></td>
<td>

### Fullstack Developer — Anti-Money Laundering
- Feature delivery, reporting, and production support
- REST APIs with **OAuth** and **JWT**
- **Stack:** PHP · CakePHP · PostgreSQL · Oracle · AWS · Atlassian

</td>
</tr>
<tr>
<td><strong>2020 — 2021</strong><br/><em>COPEXTEL</em></td>
<td>

### Computer Science Specialist B
Network administration and infrastructure support.

</td>
</tr>
<tr>
<td><strong>2019 — 2020</strong><br/><em>CITMATEL</em></td>
<td>

### Fullstack Developer — RODAS v4.0 (Accounting)
- Inventory module: warehouse, products, documents, unit converters
- Operational reports for finance workflows
- **Stack:** Java · Spring · Tomcat · Jaspersoft · PostgreSQL

</td>
</tr>
<tr>
<td><strong>2016 — 2019</strong><br/><em>Ministry of Interior University</em></td>
<td>

### Professor
Taught AI, Discrete Math, Databases, and OOP design.  
Also shipped internal faculty tools (PHP · CodeIgniter · MySQL).

</td>
</tr>
</table>

---

## Featured projects

<p align="center">
  <a href="https://github.com/ale94lko/php-cs-fixer-action">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=php-cs-fixer-action&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="php-cs-fixer-action" />
  </a>
  <a href="https://github.com/ale94lko/sql-optima">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=sql-optima&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="sql-optima" />
  </a>
</p>
<p align="center">
  <a href="https://github.com/ale94lko/llm-workbench">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=llm-workbench&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="llm-workbench" />
  </a>
  <a href="https://github.com/ale94lko/git-token-auth">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=git-token-auth&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="git-token-auth" />
  </a>
</p>
<p align="center">
  <a href="https://github.com/ale94lko/php-queue-prophet">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=php-queue-prophet&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="php-queue-prophet" />
  </a>
  <a href="https://github.com/ale94lko/pr-metadata-validator">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=ale94lko&repo=pr-metadata-validator&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&border_color=2a3f55&description_lines_count=2" alt="pr-metadata-validator" />
  </a>
</p>

| Project | What it does |
|---|---|
| [**php-cs-fixer-action**](https://github.com/ale94lko/php-cs-fixer-action) | GitHub Action to enforce PHP coding standards |
| [**sql-optima**](https://github.com/ale94lko/sql-optima) | Automated SQL optimizer & schema linter for PRs/APIs |
| [**llm-workbench**](https://github.com/ale94lko/llm-workbench) | Local-first multi-LLM prompt testing & versioning |
| [**git-token-auth**](https://github.com/ale94lko/git-token-auth) | Zero-dependency Git credential helper for shared envs |
| [**php-queue-prophet**](https://github.com/ale94lko/php-queue-prophet) | Predict worker memory leaks & queue overflow before crashes |
| [**pr-metadata-validator**](https://github.com/ale94lko/pr-metadata-validator) | Enforce branch naming & PR title rules in CI |
| [**abandoned-deps-action**](https://github.com/ale94lko/abandoned-deps-action) | Audit `package.json` for unmaintained dependencies |
| [**repo-health-score**](https://github.com/ale94lko/repo-health-score) | Community-profile health score for repositories |

---

## Skills

**Languages & platforms**  
`PHP` `TypeScript` `JavaScript` `Java` `C#` `SQL` `PL/SQL` `Shell`

**Backend & APIs**  
`REST` `OAuth` `JWT` `Yii` `CakePHP` `CodeIgniter` `Spring` `ASP.NET`

**Frontend**  
`Vue` `HTML` `CSS` `Bootstrap` `jQuery`

**Data**  
`PostgreSQL` `MySQL` `Oracle` `SQLite`

**DevOps & tooling**  
`GitHub Actions` `Git` `AWS` `Docker-friendly CI` `Atlassian`

<p align="left">
  <img src="https://img.shields.io/badge/PHP-0a1628?style=flat-square&logo=php&logoColor=7ec8b8" alt="PHP" />
  <img src="https://img.shields.io/badge/TypeScript-0a1628?style=flat-square&logo=typescript&logoColor=7ec8b8" alt="TypeScript" />
  <img src="https://img.shields.io/badge/JavaScript-0a1628?style=flat-square&logo=javascript&logoColor=e0b07a" alt="JavaScript" />
  <img src="https://img.shields.io/badge/Java-0a1628?style=flat-square&logo=openjdk&logoColor=e0b07a" alt="Java" />
  <img src="https://img.shields.io/badge/Vue.js-0a1628?style=flat-square&logo=vuedotjs&logoColor=7ec8b8" alt="Vue" />
  <img src="https://img.shields.io/badge/PostgreSQL-0a1628?style=flat-square&logo=postgresql&logoColor=7ec8b8" alt="PostgreSQL" />
  <img src="https://img.shields.io/badge/MySQL-0a1628?style=flat-square&logo=mysql&logoColor=e0b07a" alt="MySQL" />
  <img src="https://img.shields.io/badge/Oracle-0a1628?style=flat-square&logo=oracle&logoColor=e0b07a" alt="Oracle" />
  <img src="https://img.shields.io/badge/GitHub%20Actions-0a1628?style=flat-square&logo=githubactions&logoColor=7ec8b8" alt="GitHub Actions" />
  <img src="https://img.shields.io/badge/AWS-0a1628?style=flat-square&logo=amazonwebservices&logoColor=e0b07a" alt="AWS" />
  <img src="https://img.shields.io/badge/Spring-0a1628?style=flat-square&logo=spring&logoColor=7ec8b8" alt="Spring" />
  <img src="https://img.shields.io/badge/Git-0a1628?style=flat-square&logo=git&logoColor=e0b07a" alt="Git" />
</p>

---

## Education & awards

| | |
|---|---|
| **Computer Science Engineer** | Ministry of Interior University · Graduated with Honours · **GPA 4.79** (2012–2016) |
| **Teaching category** | Instructor Teacher (2017) |
| **Scientific category** | Aspiring Researcher (2018) |
| **Awards** | Gold medal — Cuban Programming Contest (2013) · Silver medal (2012) |

---

## GitHub pulse

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=ale94lko&show_icons=true&include_all_commits=true&count_private=true&theme=transparent&bg_color=0a1628&title_color=e0b07a&icon_color=7ec8b8&text_color=c9d6e2&hide_border=true&custom_title=GitHub%20Stats" alt="GitHub Stats" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ale94lko&layout=compact&langs_count=8&theme=transparent&bg_color=0a1628&title_color=e0b07a&text_color=c9d6e2&hide_border=true" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ale94lko&theme=transparent&background=0a1628&ring=e0b07a&fire=c4894a&currStreakLabel=7ec8b8&sideLabels=7ec8b8&currStreakNum=c9d6e2&sideNums=c9d6e2&dates=8fa3b8&hide_border=true" alt="GitHub Streak" />
</p>

<p align="center">
  <a href="https://github.com/ryo-ma/github-profile-trophy">
    <img src="https://github-profile-trophy.vercel.app/?username=ale94lko&theme=onedark&no-frame=true&no-bg=true&column=7&margin-w=8&margin-h=8" alt="GitHub Trophies" />
  </a>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/ale94lko/ale94lko/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/ale94lko/ale94lko/output/github-contribution-grid-snake.svg" />
    <img alt="GitHub contribution snake animation" src="https://raw.githubusercontent.com/ale94lko/ale94lko/output/github-contribution-grid-snake.svg" />
  </picture>
</p>

---

<p align="center">
  <a href="mailto:ale94lko@gmail.com">ale94lko@gmail.com</a>
  ·
  <a href="https://github.com/ale94lko">github.com/ale94lko</a>
</p>
